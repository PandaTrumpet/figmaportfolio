        ...
        <div className="max-w-[1400px] mx-auto w-full">
          ...
          >
            ...
            <p className="text-2xl md:text-3xl opacity-70 max-w-3xl leading-relaxed">
              От стратегии до запуска — создаём digital-продукты, 
              которые работают и восхищают.
            </p>
          </motion.div>
        </div>
        ...
        <div className="max-w-[1400px] mx-auto">
          ...
          >
            ...
            <p className="text-xl md:text-2xl opacity-70 max-w-3xl">
              Прозрачные цены без скрытых платежей. Выберите подходящий уровень или закажите индивидуальное решение.
            </p>
          </motion.div>
            ...
            {pricingTiers.map((tier, index) => (
              <motion.div
                ...
                className={`border-2 border-[#050608] p-8 md:p-10 relative ${
                  tier.highlighted ? 'bg-[#050608] text-[#F5EFE7]' : 'bg-[#F5EFE7]'
                }`}
              >
                {tier.highlighted && (
                ...
                <p className={`text-lg mb-6 ${tier.highlighted ? 'opacity-70' : 'opacity-60'}`}>
                  {tier.description}
                </p>
                ...
                <ul className="space-y-3 mb-8">
                  {tier.features.map((feature, i) => (
                    <li key={i} className="flex items-start gap-3">
                      ...
                      <span className={tier.highlighted ? 'opacity-90' : 'opacity-70'}>{feature}</span>
                    </li>
                  ))}
                </ul>
        ...
        <div className="max-w-[1400px] mx-auto text-center relative z-10">
          ...
          >
            ...
            <p className="text-xl md:text-2xl opacity-70 mb-12 max-w-3xl mx-auto leading-relaxed">
              Готовы к проекту, который изменит ваш бизнес? 
              Начнём с честного разговора о целях и возможностях.
            </p>
            <motion.a
              href="#contact"
...
function ServiceBlock({ service, index }: { service: typeof servicesNew[0]; index: number }) {
  const sectionRef = useRef(null);
  const isInView = useInView(sectionRef, { once: true, margin: "-100px" });

  return (
    <div ref={sectionRef} className="grid lg:grid-cols-[1fr,1.2fr] gap-12 lg:gap-20 items-start">
      {/* Content Column */}
      <motion.div
      ...
      >
        ...
        <div className="border-2 border-[#050608] bg-white p-8">
          <h4 className="text-xl mb-6 uppercase tracking-wider">Что Включено</h4>
          <ul className="space-y-3">
            {service.features.map((feature, i) => (
              <li key={i} className="flex items-start gap-3">
                <div className="w-1.5 h-1.5 bg-[#050608] flex-shrink-0 mt-2.5" />
        ...
        <div className="mt-8 bg-[#050608] text-[#F5EFE7] border-2 border-[#050608] p-8">
          <div className="flex items-end justify-between mb-6">
            <div>
              <p className="text-sm uppercase tracking-wider opacity-50 mb-2">От</p>
              <p className="text-3xl">{service.startingPrice}</p>
            </div>
            <div>
              <p className="text-sm uppercase tracking-wider opacity-50 mb-2">Срок</p>
