...
export function Portfolio() {
  ...
  const filteredProjects = projects.filter((project) => {
    const industryMatch = selectedIndustry === 'All' || project.industry.includes(selectedIndustry);
    const typeMatch = selectedType === 'All' || project.type.includes(selectedType);
        ...
        <div className="max-w-[1600px] mx-auto w-full">
          ...
          >
            ...
            <p className="text-2xl md:text-3xl opacity-70 max-w-3xl leading-relaxed">
              Работы, которыми мы гордимся. Каждый проект — это история успеха клиента.
            </p>
          </motion.div>
        </div>
      ...
      <section className="px-6 md:px-12 lg:px-20 py-12 border-t-2 border-b-2 border-[#050608] bg-white sticky top-0 z-40">
        <div className="max-w-[1600px] mx-auto">
          <div className="grid md:grid-cols-2 gap-8">
            {/* Industry Filter */}
            <div>
              <p className="text-sm uppercase tracking-wider opacity-50 mb-4">Industry</p>
            ...
            {/* Type Filter */}
            <div>
              <p className="text-sm uppercase tracking-wider opacity-50 mb-4">Type</p>
              <div className="flex flex-wrap gap-3">
                {types.map((type) => (
                  <button
                    key={type}
        ...
        <div className="max-w-[1600px] mx-auto">
          ...
          {filteredProjects.length === 0 && (
            <div className="text-center py-20">
              <p className="text-2xl opacity-40">No projects found with these filters</p>
              <button
                onClick={() => {
        ...
        <div className="max-w-[1400px] mx-auto text-center relative z-10">
          ...
          >
            ...
            <p className="text-xl md:text-2xl opacity-70 mb-12 max-w-3xl mx-auto leading-relaxed">
              Каждый успешный проект начинается с разговора. 
    ...
    >
      <div className="border-2 border-[#050608] bg-white overflow-hidden hover:shadow-[12px_12px_0px_0px_rgba(5,6,8,1)] transition-all duration-300">
        ...
        <div className="p-6">
          ...
          <p className="text-lg opacity-70 leading-relaxed mb-6">{project.description}</p>
          
          {/* Tags */}
          <div className="flex flex-wrap gap-2">
            {project.industry.slice(0, 2).map((ind) => (
              <span key={ind} className="text-sm px-3 py-1 bg-[#F5EFE7] border border-[#050608] opacity-70">
                {ind}
              </span>
            ))}
            {project.type.slice(0, 1).map((t) => (
              <span key={t} className="text-sm px-3 py-1 bg-[#050608] text-[#F5EFE7] border border-[#050608]">
                {t}
              </span>
            ))}
          </div>
...
function CaseStudyBlock({ project, index }: { project: typeof projects[0]; index: number }) {
      ...
      >
        ...
        <div className="mb-12">
          <div className="flex items-center gap-6 mb-6">
            ...
            <div>
              <h3 className="text-4xl md:text-5xl lg:text-6xl mb-2">{project.title}</h3>
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 mb-12">
          ...
          >
            <h4 className="text-2xl mb-4 uppercase tracking-wider">Challenge</h4>
            <p className="text-lg opacity-70 leading-relaxed">{project.caseStudy.challenge}</p>
          </motion.div>
          ...
          >
            ...
            <p className="text-lg opacity-90 leading-relaxed">{project.caseStudy.solution}</p>
          </motion.div>
        </div>
        ...
        {project.caseStudy.images && project.caseStudy.images.length > 0 && (
          ...
          >
            {project.caseStudy.images.map((img, i) => (
              <div key={i} className="aspect-[16/10] border-2 border-[#050608] overflow-hidden">
                <ImageWithFallback
                  src={img}
