...
export function ClientStoriesSection({ stories }: { stories: ClientStory[] }) {
  ...
  const titleId = "client-stories-title";
  const descId = "client-stories-desc";
  const titleText = t("clientStories.title");
  ...
  return (
    ...
    >
      ...
      <PageContainer className="relative z-10 text-[#F5EFE7]">
        ...
        {/* Cards */}
        <div className="space-y-14 md:space-y-16" role="list">
          {stories.map((story, index) => (
            <div key={`${story.company}-${index}`} role="listitem">
              <ClientStoryCard story={story} index={index} />
            </div>
          ))}
        </div>
      </PageContainer>
