...
function Block({
  ...
  return (
    ...
    >
      {/* Glow behind card */}
      <div
        className="pointer-events-none absolute -inset-0.5 -z-10 rounded-[32px] bg-[radial-gradient(circle_at_top,_rgba(56,189,248,0.4),transparent_60%)] opacity-0 group-hover:opacity-100 blur-2xl transition-opacity duration-700"
        aria-hidden="true"
      />
      ...
      <div className={`${reason.reverse ? "md:order-1" : ""} relative`}>
        <div
          className="pointer-events-none absolute -inset-10 bg-[radial-gradient(circle_at_bottom,_rgba(244,114,182,0.4),transparent_65%)] opacity-60 blur-3xl"
          aria-hidden="true"
        />
...
export function WhyUsSection() {
        ...
        <div className="relative">
          ...
          >
            <motion.div
              ...
              transition={{ duration: 0.7 }}
            >
              {/* (пустой блок оставил как есть, чтобы не менять структуру/стили) */}
            </motion.div>
  ...
  md:leading-[1.15] text-slate-50 mb-3"
          ...
          <div className="space-y-16">
            {reasons.map((reason, index) => (
              <Block key={reason.id} reason={reason} index={index} />
            ))}
          </div>
        </div>
      </PageContainer>
