...
export function CaseStudyBlock({
  ...
  const t = useTranslations("caseStudyBlock");
...
const posterSource = project.imageBig || project.imageLittle;

const posterSrc =
  typeof posterSource === "string" ? posterSource : posterSource.src;
  return (
    <motion.article
      ref={sectionRef}
      initial={reduce ? false : { opacity: 0, y: 28 }}
      animate={reduce ? { opacity: 1 } : isInView ? { opacity: 1, y: 0 } : {}}
    ...
    >
      <div className="relative rounded-3xl border border-white/10 bg-gradient-to-b from-[#060A13] via-[#050816] to-[#02030A] shadow-[0_22px_70px_rgba(0,0,0,0.75)] overflow-hidden">
        <div
          className="pointer-events-none absolute inset-0 bg-gradient-to-br from-[#3A7BFF22] via-transparent to-transparent opacity-70"
          aria-hidden="true"
        />
            ...
            <div className="flex items-start gap-4 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <div
                  ...
                  aria-label={t("tagsAria", { title: project.title })}
                >
                  ...
                  {project.type?.slice(0, 2).map((typeLabel: string) => (
                    <span
                      key={typeLabel}
                      className="
                        text-[12px] md:text-[13px]
                ...
                <div className="aspect-[16/9]">
                  {project.video ? (
                    <video
                      ...
                      preload="metadata"
                      poster={
                        typeof (project.imageBig || project.imageLittle) ===
                        "string"
                          ? project.imageBig || project.imageLittle
                          : (project.imageBig || project.imageLittle).src
                      }
              ...
              {project.caseStudy.images.length > 0 && (
                <AnimatePresence initial={false}>
                  <motion.div
                    ...
                    aria-label={t("galleryAria", { title: project.title })}
                  >
            ...
            <div className="lg:col-span-5">
              <div className="rounded-2xl border border-white/10 bg-white/5 p-5 md:p-6 backdrop-blur-xl">
                ...
                <div
                  className="space-y-3"
                  role="list"
                  aria-label={t("resultsListAria", { title: project.title })}
                >
                ...
                <div className="mt-5">
                  <Link
                    ...
                    "
                    aria-label={t("ctaAria", { title: project.title })}
                  >
                    <span>{t("cta")}</span>
                    <ArrowRight
          ...
          <AnimatePresence initial={false}>
            ...
            >
              <div className="grid lg:grid-cols-2 gap-4">
                <section
                  className="rounded-2xl border border-white/10 bg-white/5 p-5 md:p-6 backdrop-blur-xl"
                  aria-labelledby={`case-study-challenge-${project.id}`}
                >
                  <h5
                    id={`case-study-challenge-${project.id}`}
                    className="text-[11px] md:text-[12px] leading-[1.2] uppercase tracking-[0.22em] text-[#C7CEDF] mb-3"
                  >
                    {t("challengeTitle")}
                  </h5>
                ...
                <section
                  className="rounded-2xl border border-white/10 bg-gradient-to-b from-[#060A13] via-[#050816] to-[#02030A] p-5 md:p-6"
                  aria-labelledby={`case-study-solution-${project.id}`}
                >
                  <h5
                    id={`case-study-solution-${project.id}`}
                    className="text-[11px] md:text-[12px] leading-[1.2] uppercase tracking-[0.22em] text-[#C7CEDF] mb-3"
                  >
                    {t("solutionTitle")}
                  </h5>
