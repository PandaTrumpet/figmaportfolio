...
export function ProjectsGridSection({ projects }: { projects: Project[] }) {
  const t = useTranslations("projectsGridSection");
    ...
    >
      ...
      <PageContainer className="relative z-10 max-w-[1200px]">
        <div className="sr-only">
          <h2 id="projects-grid-heading">{t("title")}</h2>
        </div>

        <div className="mb-10 md:mb-14">
          <div
            className="inline-flex items-center gap-3 rounded-full border border-white/10 bg-white/5 px-5 py-2.5 backdrop-blur-xl"
            aria-label={t("counterAria", { count: projects.length })}
          >
            <span
              className="h-2 w-2 rounded-full bg-gradient-to-r from-[#3A7BFF] to-[#4CC2FF]"
              aria-hidden="true"
            />
            <p className="text-sm md:text-base text-[#C7CEDF]">
              {t("counterLabel")}{" "}
        ...
        {projects.length > 0 ? (
          ...
          <div className="text-center py-20" role="status" aria-live="polite">
            <p className="text-2xl text-[#C7CEDF] opacity-50">{t("empty")}</p>
          </div>
        )}
      </PageContainer>
    </section>
