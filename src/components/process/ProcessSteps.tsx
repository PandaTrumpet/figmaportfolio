...
// Лучше импортировать тип из data или отдельного файла types
import type { ProcessStep as ProcessStepType } from "@/src/data/processData";

export function ProcessSteps({ steps }: { steps: ProcessStepType[] }) {
  const t = useTranslations("processe");

  return (
    <section
      aria-labelledby="process-steps-heading"
      aria-describedby="process-steps-subtitle"
      className="relative  pt-10 pb-10
        md:pt-14 md:pb-14
        lg:pt-16 lg:pb-16 overflow-visible"
    >
  ...
  text-[clamp(0.98rem,0.55vw+0.88rem,1.15rem)] max-w-3xl mx-auto "
          ...
          {/* ✅ Семантический список шагов */}
          <ol className="space-y-4 sm:space-y-6 md:space-y-8 lg:space-y-10 xl:space-y-12">
            {steps.map((step, index) => (
              <li key={`${step.number}-${index}`} className="list-none">
