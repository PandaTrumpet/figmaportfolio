...
//   // показываем "+ещё N" (если хочешь исключить первый пункт — поменяй на Math.max(0, step.deliverables.length - 1))
...
//                     <span className="text-[12px] md:text-[13px] leading-[1.2] text-[#D7DDEA]">
//                       {step.duration}
//                     </span>
//                   </div>
//                 </div>
...
//                 <div
//                   className="
//                     mt-5 inline-flex flex-wrap items-center gap-2
//                     rounded-full border border-[#4CC2FF33] bg-[#4CC2FF1a]
//                     px-4 py-2
...
// тип лучше импортировать единый
import type { ProcessStep as ProcessStepType } from "@/src/data/processData";
...
export function ProcessStep({ step }: { step: ProcessStepType }) {
  ...
  // ✅ показываем "+ещё N" (первый пункт уже показан как “Результат этапа”)
  const hiddenCount = Math.max(0, (step.deliverables?.length ?? 0) - 1);
  ...
  return (
    <motion.article
      ...
      transition={{ duration: 0.6, ease: EASE }}
      className="relative"
      aria-label={`${t("step.aria.stepLabel")} ${step.number}: ${step.title}`}
    >
      <div className="grid grid-cols-[28px,1fr] gap-5 md:gap-7">
        {/* Timeline rail */}
            ...
            <div className="flex flex-col lg:flex-row lg:items-start gap-6 lg:gap-10">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div
                  className="
                    ...
                    text-[#EAF2FF]
                  "
                  aria-label={t("step.aria.resultLabel")}
                >
                  {t("step.resultPrefix")}
                  <span className="opacity-80">{step.deliverables?.[0]}</span>
                </div>
                ...
                <div className="mt-6">
                  ...
                  >
                    ...
                    <span>
                      {open
                        ? t("step.hideDetails")
                        : `${t("step.showDetails")}${hiddenCount ? ` (+${hiddenCount})` : ""}`}
                    </span>
                    <motion.span
                      aria-hidden="true"
                      animate={{ rotate: open ? 180 : 0 }}
                  ...
                  <motion.div
                    id={panelId}
                    role="region"
                    aria-label={t("step.aria.detailsRegion")}
                    initial={false}
                    animate={
                      open
                        ? { height: "auto", opacity: 1 }
              ...
              <div className="hidden lg:block shrink-0">
                <div className="relative w-[200px]">
                  <div
                    aria-hidden="true"
                    className="pointer-events-none absolute -inset-6 rounded-3xl bg-[radial-gradient(circle_at_top,_rgba(76,194,255,0.18),_transparent_70%)] blur-2xl opacity-70"
                  />
